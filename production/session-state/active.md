...
**Current Stage Note:** S3-06 mechanically implemented in the Codex lane (AC-01–06 proven), but the N=1 human-play attempt (Brian, 2026-05-30) **FAILED the feel gate for presentation-readability reasons** — the slice reads as Unity greybox/debug scaffolding, not a playable classic-MMO-descended gothic slice (compounded by a real bug: the legacy M2 combat debug HUD bleeding over the S3 objective loop). Per **DECISIONS.md D020 (Locked, 2026-05-30)**: product owner has decided to adopt more EQ-style **play legibility** ("readability, not cosplay") than the current art bible allows — a pillar-level P1 amendment. This triggers (a) a formal `design/art/art-bible.md` revision (Sections 1/5/7, banner added, frozen-for-revision), and (b) a new **Sprint 4: EQ-Readable Presentation Slice** milestone inserted before M4/M5. S3-06 is **mechanically complete, feel gate NOT PASSED**; Sprint 3 closes as mechanically successful but feel-unvalidated. **Next deliberate sessions (NOT yet started):** (1) art-bible revision via the `art-director` agent against the amended pillar; (2) Sprint 4 slate authoring after the revision lands. **Open Codex-lane item:** the mid-playtest M2-HUD-hide edit in `M2SingleTrashMedLoopController.cs` is approved to KEEP, scoped as an S3-06 presentation-readiness bug fix (Codex records it in the story/evidence, not a silent floating change). Main = origin/main; Sprint 3 slate stories S3-01..S3-05 done, S3-06 mechanically done / feel-failed.
**Project Stage:** Pre-Production — Sprint 3 closing (feel-unvalidated) → Sprint 4 (EQ-Readable Presentation Slice) re-plan pending art-bible revision
...
## Session Extract — /story-done 2026-06-07 (S4-00) + Sprint 4 governance

- **Sprint 4 (EQ-Readable Presentation Slice) is the active sprint.** QA plan landed + pushed (`d1816fd`); Sprint 3->4 reconciliation pushed (`b2e5a6b`, `origin/main` now there). Slate S4-00..S4-06 reconciled into `sprint-status.yaml`.
- **S4-00 closed COMPLETE WITH NOTES (2026-06-07).** ux-designer design pass; deliverable = `production/qa/evidence/s4-00-hud-thresholds-evidence.md` (committed `e244460`, local/unpushed at time of writing). Resolved the art-bible §7.10 deferred numbers: vitals bar-height floor 6px@1080p (3px retired), panel-fill opacity floor 65% (45% retired), cast-bar placement (top edge 70% viewport, clears the 40-60% band), and the 5-state shape-primary con-glyph set (dash/down-tri/square/up-tri/double-up-tri, world-palette confirm-colors, no red/green). Every value is a reasoned floor or an open-question-with-method (OQ-1..7); no TBD.
- **Forward-assigned validations:** OQ-1..5 (in-engine bar read, panel >=3:1 measurement, cast-bar screenshot) -> S4-02/S4-04 own them; OQ-6/7 (con-glyph 5/5 discriminability render + human naming test) -> **S4-03**, by explicit product-owner decision (non-strict closure).
...
## Session Extract — /story-done 2026-06-09 (S5-03 + S5-01/S5-02 reconciliation)

- **The production pipeline went live end-to-end this session**: Unity MCP bridge (stdio, port 6400) + Blender MCP installed and connected; 4 AI-generated albedo textures intaken (Art Judge PASS), URP materials + practical lighting + NPC placeholder palette + full set dressing applied to the Sexton's Court **through the live editor** (zero hand-edited scene YAML).
- **S5-03 closed COMPLETE WITH NOTES** (6/6 ACs). Evidence: `production/qa/evidence/s5-03-produced-art-evidence.md` — manifest complete (4 material sets, lighting rig <=4 budget, 5 props at cap, each §6.4-caused), **CD gate PASS WITH ADJUSTMENTS** (caught the lantern co-lighting Morrvik+door = soft routing; moved to (-1.5,0,-3.5), parity shown in pixels), **M2 preservation 3/3 PASS** (batchmode, builder skipped), perf snapshot all-PASS vs the S5-02 budgets, **[F1] tuple at final scene state** (`bae11334` @ `67fe9c4`; re-hashed after the CD move — stale-anchor trap avoided), ProjectSettings linear-intensity ADOPTED deliberately (gated lighting depends on it).
- **S5-01 + S5-02 reconciled done** (delivered `19193eb`/`3cab743` via the Codex lane, consumed by S5-03; bookkeeping was deliberately trailing per the product-owner pixels-first directive). `completed_stories` 1→4/6.
- **Dependency recorded**: `com.coplaydev.unity-mcp` (MIT, editor-only) in Allowed Libraries + Packages committed.
- **Carried forward**: CD "cursed gothic" tone note (sky/grime — next art pass); NPC body production (design brief done — Morrvik/vendor static-mesh route, ghouls primitive-improve route — execution routed separately); final budget lock at the S5-05 play profile.
- **Lane state**: Codex held cleanly all session (its review caught the cobble 1024 spec deviation — corrected `32f6451`). Scene concurrency now CLEAR for S5-04.
- **Next recommended:** **S5-04** — Codex lane resumes: rebase `codex/s4-01-play-camera-debug-hud-isolation` onto origin/main, re-validate, land the legibility floor. Then **S5-05** (the third feel gate).

## Session Extract — /story-done 2026-06-09 (S5-04; PR #10 merged ab031b4)

- **S5-04 closed COMPLETE WITH NOTES (5/5 ACs)** — merged via PR #10. The story grew a real fix: the first manual behavior check **FAILED** (auto-pull near Morrvik; nothing rendering), which diagnosed three findings — **(F-A) the legacy M2 controller stomped Play Mode presentation every frame** (near-black fog + ambient override + floor repaint over the produced cobble; every prior Play session including BOTH feel-fails was judged through this stomp), (F-B) M2 proximity body-pull next to Morrvik, (F-C) unscaled tiny prompt. Fixed via `ShouldSuppressLegacyM2DuringObjectiveFreeWalk` (explicit `Application.isBatchMode || _smokeRunning` scenario discrimination — human free-walk only; combat re-enables everything) + prompt scaling. Re-checks: manual **PASS WITH NOTES**, post-fix M2 preservation **3/3 PASS**, 189/189, format ×2, zero scene/settings/package drift. Main-lane review APPROVE.
- **Sprint 5 at 5/6.** S5-05 (the re-targeted feel gate) is now **Ready** — all dependencies done. It will be the **first feel check ever run on an un-stomped Play Mode** (load-bearing fact; prior feel-fails partially judged the stomp, not the content).
- **Notes routed forward**: player avatar/orientation read → character/NPC body story (spec drafted at `production/qa/evidence/s5-character-npc-animation-spec.md`, **uncommitted**, awaiting commit-go + the Q4 decision bodies-before/after-S5-05); ability surface (harness = auto-attack + Smite only; combat-core names Lesser Heal/Bash/Defensive Prayer) → combat ability-surface backlog story.
- Untracked leftovers deliberately kept out of the PR (Codex worktree): Unity `_Recovery` files + the old parked S4-01 evidence.
- Next recommended: **S5-05** — Brian's N=1 human-play gate on the produced Sexton's Court (F1 tuple compare vs S5-03 evidence; F2/F3/F7 fences; place-read AND state-read; first-pass ≠ final-art). Then Sprint 5 close-out.
