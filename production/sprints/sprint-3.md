...
Proposed 6-story slate, dependency-ordered. Story IDs use the `S3-NN` scheme — Sprint 3 is single-milestone, so the Sprint 2 `S2-M2-NN` / `S2-M3-NN` per-milestone prefix is not needed. IDs are a proposal for `/create-stories`; they are not committed until the slate is opened.
