...
### 2026-06-07 — [FEEL][PROCESS] Greybox is not a feel-validation bar for this product

**Context:** Two consecutive human-play attempts failed demo-readiness for the same reason. S3-06 (2026-05-30) failed as "reads as Unity greybox/debug scaffolding"; D020 routed the fix to an art-bible legibility revision + a HUD-first Sprint 4. After S4-00 (HUD thresholds) and the S4-01 camera/debug-HUD-isolation pass landed, Brian played again (N=1, 2026-06-07) and the verdict was unchanged: "still doesn't even resemble a playable demo." D016 had asserted (lines 587–590) that the presentation minimum is greybox — "feel is carried by interactivity, spatial readability, and feedback, not texture fidelity." That premise was a hypothesis; it has now been tested twice against the only playtester who matters for this slice and failed both times.

**Lesson:** For this product owner, on this gothic-MMO-descended "cursed city" fantasy, fidelity is not separable from feel — a district reading as debug scaffolding structurally cannot deliver "cursed" against the EQ-Classic reference frame the slice invokes. Greybox validated that the systems were *assembled*; it could not validate that the slice is the *game*. Do not defer art to validate feel here. The corrected sequence (D021): design the place → produce ONE representative area first-pass (bounded by an asset manifest) → re-validate. Two corollaries: (1) a feel-fail has axes — separate world-read (place) from state-read (HUD legibility); the S3-06/S4-01 fails had both, so produced art alone is necessary-not-sufficient and the legibility floor must ride along. (2) Never produce art for a place that was only blocked out for mechanics — design intent (art-bible §6.4) gates the asset pass, or the result is generic atmosphere that also fails Pillar 1.

**Evidence:** `DECISIONS.md` D021 (the produced-art pivot amending D016) and D016 lines 587–590 (the falsified greybox premise); `production/qa/evidence/s4-01-play-camera-evidence.md` (the failed N=1 verdict); `tests/evidence/S3-06/` (first fail); the four 2026-06-07 lead assessments (creative-director / art-director / producer / technical-director).

**Promotion status:** open — candidate for promotion to a governance/process rule if a third feel-checkpoint confirms the design-place-then-produce-then-validate sequence. Until then it is the standard for any future "defer art to validate feel" proposal.

### 2026-06-07 — [GLOBAL][TEST] A "seam" proven by a string annotation is not a seam

**Context:** S3-04 added the first D017 application: the vendor adapter `M3VendorInteractTarget` carries a `/// SERVER-AUTH-INTENT` doc comment, and its verification runner asserts the literal string `SERVER-AUTH-INTENT` is present in source (`vendor_authority_annotation_present`). At S3-04 review this was praised as "runner-self-asserted, regression-protected — the proof D017 was worth writing." A later independent code review (2026-06-07, finding F4) corrected that: the adapter calls `vendor.TrySellRecoveredSalvage(...)` **directly** from `TryInteract` — there is no command/intent value object. D017 requires that economy / inventory / objective / save / combat / faction mutations flow through "a command/intent a server could validate later." A string-presence check proves *annotation awareness*, not a *migration path*. The seam is documentary, not structural — and the original S3-04 review over-credited it.

**Lesson:** When a decision (D017) requires a *structural* seam, the evidence must exercise the seam's structure, not the presence of a comment naming it. A runner that greps for `SERVER-AUTH-INTENT` proves the words exist; it does not prove the mutation can move server-side without redesign. This is the "green evidence, false premise" failure applied to architecture-intent: the check is real, passes honestly, and proves the wrong thing. Going forward the D017 verification pattern must require a real command/intent object (e.g. a `VendorSellSalvageIntent` the adapter routes through, which a server could validate), and the runner must assert the mutation flows through that object — not that a string is present. T1 may still execute the intent locally (D017 permits local execution); the seam must nonetheless be structural. This also stands as a reviewer-humility note: the S3-04 verdict called a comment a seam, and the correction came from an independent review applying the same evidence discipline to the reviewer's own conclusion.

**Evidence:** `Assets/Scripts/M3VendorInteractTarget.cs:12` (the comment), `:74` (the direct `TrySellRecoveredSalvage` call); `Assets/Editor/GravenspireS3PlayerDrivenVendorVerificationRunner.cs:152` (the string-presence check); `DECISIONS.md` D017; 2026-06-07 code review finding F4.

**Promotion status:** open — promote to `.claude/rules/game-dev-governance.md` (or a dedicated D017 verification-pattern note) when economy / inventory / save / faction implementation work resumes and a real command/intent seam is built. Until then this is the standard for any future D017-mandatory-system evidence.

### 2026-06-07 — [GLOBAL][TEST] Absence proven by name-scan is vocabulary-bound, not structural

**Context:** Several Sprint 3 runners prove the *absence* of a forbidden surface by scanning scene object names, component type names, and event names for known marker strings — e.g. the M3 end-to-end runner checks for `FactionConsequence`, `FactionReaction`, `FactionStandingConsequence`, `FactionSink` and reports absence when none match; the S3-04 buy-side scan checks one adapter file for `TryPurchaseFixedVendorGood`. A 2026-06-07 code review (findings F5/F6) noted these prove "absence of *known* marker vocabulary," not "absence of all such behavior." A faction path named `StandingDeltaApplier` or `CourtReputationEmitter`, or a buy-side `IPlayerInteractTarget` on another GameObject, would pass the scan while the forbidden behavior exists.

**Lesson:** A name-scan absence proof is bounded by the reviewer's vocabulary, not by the codebase's actual behavior — it cannot prove a negative across unknown names. Label such evidence honestly as "known-vocabulary structural absence" (not "no buy-side path anywhere" / "no faction consequence anywhere"), and narrow the claim to what was actually scanned (e.g. "M3VendorInteractTarget is sell-only"). To prove absence *structurally* rather than by vocabulary, the forbidden surface must be required to implement a marker interface or register in a known registry — then "no implementers / no registered entries" is a real negative. Before real faction-consequence (M5) or economy buy-side work lands, require consequence/mutation surfaces to carry such a marker so absence is provable by structure, not string search.

**Evidence:** `Assets/Editor/GravenspireM3EndToEndObjectiveLoopVerificationRunner.cs` (faction-marker name scan); `tests/evidence/S3-04/buy-side-absence-scan-20260530.txt` (single-file buy-side scan); 2026-06-07 code review findings F5/F6.

**Promotion status:** open — promote to a marker-interface/registry requirement when M5 faction-consequence or economy buy-side work begins; until then, absence claims in evidence must be labeled vocabulary-bound and scoped to what was scanned.
