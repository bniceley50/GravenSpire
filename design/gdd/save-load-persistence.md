...
- **Combat Core** (§7, graph-listed at [systems-index.md:35](systems-index.md)) — **Hard (graph).** Participates indirectly via Character Progression (settled XP updates and progression-derived permanent health/mana maxima for load hydration), NPC System (durable source lifecycle state for defeated sources), and Death & Corpse Recovery (→ WS's `CorpseRecord`). Save writes consult Character Progression's `ProgressionSaveBarrier` and NPC System's `NpcSourceLifecycleSaveBarrier` before serializing XP and source state; Combat actor hydration/build occurs after Character Progression publishes `ProgressionBaselineSnapshot`; Combat Core's own direct persistence surface, if any, remains defined in its own GDD.
...
**Total: 39 criteria. 36 T1-blocking, 3 advisory-at-T1 (H-SL-R10a, H-SL-R10b, H-SL-FM-MigrationStepFailed — all promote to T1-blocking at first v1→v2 schema bump).**
