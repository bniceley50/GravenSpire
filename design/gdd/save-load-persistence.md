...
| **Character Progression** (§8) | Deserialised Character Progression state on load: `progression_schema_version`, `class_id`, `current_level`, `total_xp`, and `spell_eligibility_tier`; hydration validation result and `ProgressionBaselineSnapshot` before Combat actor hydration. | Current whitelisted Character Progression state on each save trigger. | Character Progression owns schema, validation, and permanent baseline computation; Save/Load owns serialisation, HMAC, versioning, load ordering, and failure surfacing. | **Hard** |
...
| **Character Progression** (§8) | S/L depended on by | Whitelisted progression state (`progression_schema_version`, `class_id`, `current_level`, `total_xp`, `spell_eligibility_tier`) persisted and hydrated; `ProgressionBaselineSnapshot` produced before Combat actor hydration. | **Hard** | Character Progression owns schema, validation, and permanent baseline computation; Save/Load owns serialisation, HMAC, versioning, load ordering, and failure surfacing. |
...
- **Combat Core** (§7, graph-listed at [systems-index.md:35](systems-index.md)) — **Hard (graph).** Participates indirectly via Character Progression (XP updates and progression-derived permanent health/mana maxima for load hydration) and Death & Corpse Recovery (→ WS's `CorpseRecord`). Combat actor hydration/build occurs after Character Progression publishes `ProgressionBaselineSnapshot`; Combat Core's own direct persistence surface, if any, remains defined in its own GDD.
...
**Total: 37 criteria. 34 T1-blocking, 3 advisory-at-T1 (H-SL-R10a, H-SL-R10b, H-SL-FM-MigrationStepFailed — all promote to T1-blocking at first v1→v2 schema bump).**
