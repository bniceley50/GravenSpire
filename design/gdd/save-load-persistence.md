...
1. **Save state categories.** Four logical categories are persisted: **(a) Player State** — character identity (name, class, appearance tokens, ADR-0004 `local_character_id`), Character Progression's whitelisted state (`progression_schema_version`, `class_id`, `current_level`, `total_xp`, `spell_eligibility_tier`), class-owned abilities if a future Class Design GDD authors them, equipped and carried inventory, and currency. Character Creation's T1 first-save payload adds an **initial character seed** subrecord containing `local_character_id`, `creation_schema_version`, `starting_class_id`, appearance token ids, `onboarding_eligible`, `onboarding_intro_state`, `starting_equipment_template_id`, empty carried-inventory state, and `starting_faction_reputation_baseline`. Per ADR-0004, Save/Load persists the validated seed and invokes required first-save materializers before the first successful write; Character Progression materializes `CharacterProgressionSaveState`, and Inventory & Item Economy materializes `InventorySaveState` from `starting_equipment_template_id`, empty `carried_inventory`, and active `local_character_id` context before bytes are written, not during first load. `creation_schema_version` is a Character Creation schema marker in the first-save payload, but its final placement (Player State vs. Session Metadata vs. a dedicated initial-record category) remains open in [character-creation.md](character-creation.md) §Open Questions before implementation; **(b) World State** — `PlayerZoneMembership` (zoneId + `Vector3` position + zoneType), `CorpseRecord` (zoneId + `Vector3` position + `expiry_timestamp_utc`) if one exists, NPC records (`npcId`, `scheduleStateId`, `routeProgress`, `availabilityState`, `knownNameState`, `lastEvaluatedTimestampUtc`; NPC System owns the schema), ADR-0001 NPC-owned progression source lifecycle records (`zoneId`, `defeated_source_ref`, `source_lifecycle_token`, lifecycle state, and respawn/availability timing fields; NPC System owns the schema), faction state (reputation, faction control of zones), event-log entries already committed; **(c) Session Metadata** — `last_exit_timestamp_utc` (UTC epoch seconds per [world-structure.md](world-structure.md) Rule 13), save-format version stamp, HMAC signature; **(d) Player-Authored Strings** — character name and any other player-typed strings, length-bounded and sanitised on load. Explicitly NOT saved (per [.claude/rules/save-integrity.md](../../.claude/rules/save-integrity.md) §What Saves): engine internals, derived/cached values, runtime handles.
...
8a. **Downstream save-stability barriers.** ADR-0002 is authoritative for the save-stability barrier protocol. Before `Writing` reads a downstream-owned payload, Save/Load must call that system's declared save-stability hook when one exists. Character Progression's hook is `ProgressionSaveBarrier`: it must settle same-frame Combat kill-credit dispatch, XP transactions, level-up chains, and future XP adjustments before Save/Load reads `CharacterProgressionSaveState`. NPC System's matching hook is `NpcSourceLifecycleSaveBarrier`: it must settle same-frame Combat death/despawn outcomes into NPC-owned `NpcSourceLifecycleRecord` data before Save/Load reads NPC world state. Save/Load may serialize only post-barrier payloads; it never writes post-award XP unless the defeated source lifecycle state is also save-stable. If either member of the `xp_source_lifecycle_consistency` barrier group returns `Unresolved` or `Failed` within its bounded budget or the transition-save deadline, Save/Load transitions to `WriteFailed`, emits `SaveFailedEvent(DownstreamSaveBarrierUnresolved)`, and writes no bytes.

9. **Session resume sequencing.** The hard upstream ordering rule from [world-structure.md](world-structure.md) Rule 13 + H-CR-13b remains: **`SessionResumeEvent(real_elapsed_seconds, last_exit_timestamp_utc)` must fire before any `ZoneActiveEvent`** in the session-load sequence. Save/Load's load path must therefore deliver `last_exit_timestamp_utc` to WS early enough that WS can publish `SessionResumeEvent` before publishing the first `ZoneActiveEvent`. ADR-0003 adds one downstream hydration-order lock: during `Resuming`, Save/Load hydrates and validates Character Progression first, obtains `CombatProgressionBaselineSnapshot` for the active `local_character_id`, and only then may Combat Core hydrate or build the player combat actor using that snapshot's `combat_actor_level` and permanent health/mana maxima. Other gameplay-state ordering remains downstream-owned: each subscriber's Rule 13 catch-up handler specifies state-readiness requirements in its own GDD, and Save/Load's load path must satisfy those declared requirements before gameplay is enabled.
...
14. **First-run vs. file-missing distinction.** Save/Load distinguishes two cases that both present as "no save file at the expected location":
    - **First-run path** — a character's save slot has never been initialised (new game / new character). The load request resolves to *"no-save-to-load"*; the system accepts Character Creation's validated `InitialCharacterRecord`, invokes ADR-0004 required first-save materializers including `CharacterProgressionFirstSaveMaterializer` and `InventoryFirstSaveMaterializer`, then the first save trigger writes the complete initial record atomically. **Not an error; no `LoadRejected` event emitted.**
...
| `Writing` | Write request dequeued from `Idle` (or from the mutex queue as a follow-on) | `SaveWriteConfirmed` emitted → `Idle`; or fatal write error, including unresolved downstream save-stability barrier or first-save materialization failure, → `WriteFailed` | For a first successful save, invoke ADR-0004 required first-save materializers before reading/writing the initial record, including Character Progression's `CharacterProgressionFirstSaveMaterializer` and Inventory & Item Economy's `InventoryFirstSaveMaterializer`. For normal saves, invoke ADR-0002 declared downstream save-stability hooks before reading payloads, including Character Progression's `ProgressionSaveBarrier` and NPC System's `NpcSourceLifecycleSaveBarrier`; then read current in-memory Player State, World State, Session Metadata (including WS-provided `last_exit_timestamp_utc`, `PlayerZoneMembership`, `CorpseRecord` if present); serialise in version-stamp-first order; compute HMAC over full payload; commit atomically per Rule 7. Write-attempt context (trigger type, timestamp) held in transient memory for the outcome event. |
...
| **Character Creation** (§6) | Deserialised Player State on load (initial character record hydration). | ADR-0004 validated `InitialCharacterRecord` with `local_character_id` on first save. | Character Creation owns the character-record schema and identity generation; Save/Load owns persistence and active-record identity context. | **Hard** |
| **Character Progression** (§8) | Deserialised Character Progression state on load: `progression_schema_version`, `class_id`, `current_level`, `total_xp`, and `spell_eligibility_tier`; hydration validation result and ADR-0003 `CombatProgressionBaselineSnapshot` before Combat actor hydration. | ADR-0004 `CharacterProgressionFirstSaveMaterializer`; ADR-0002 `ProgressionSaveBarrier`; current whitelisted Character Progression state on each save trigger. | Character Progression owns schema, validation, permanent baseline computation, materialization, and save-eligible stability; Save/Load owns serialisation, HMAC, versioning, active-record identity context, load ordering, and failure surfacing. | **Hard** |
| **Inventory & Item Economy** (§9) | Deserialised Inventory state on load (items, currency, faction tokens). | ADR-0004 `InventoryFirstSaveMaterializer`; current Inventory state on each save trigger. | Inventory owns item schema, first-save item materialization, hydration validation, and current state. Save/Load owns serialisation, active-record identity context, first-save orchestration, and failure surfacing. | **Hard** |
| **NPC System** (§4) | Deserialised NPC record payload and NPC-owned progression source lifecycle records during `Resuming`; `LoadRejected(HydrationFailed)` routing if synchronous NPC hydration returns `NpcHydrationFailed`. | `NpcSourceLifecycleSaveBarrier`; current NPC-owned `NpcRecord` and `NpcSourceLifecycleRecord` data on each save trigger; synchronous NPC hydration validation result on load. | NPC System owns the NPC schema, source lifecycle schema, and validation; Save/Load owns serialisation, HMAC, versioning, and failure-class routing. | **Hard** |
...
- **Combat Core** (§7) participates in saves *indirectly* — XP updates flow through Character Progression after Character Progression's `ProgressionSaveBarrier` settles pending kill-credit dispatch; NPC source death/despawn state flows through NPC System after `NpcSourceLifecycleSaveBarrier`; deaths flow through Death & Corpse Recovery → WS's `CorpseRecord`. During load, Combat Core's player actor hydration/build is sequenced after Character Progression validation so Combat can validate or initialize runtime `current_health` and `current_mana` against ADR-0003 `CombatProgressionBaselineSnapshot` (`combat_actor_level`, permanent health/mana maxima). Combat Core's own direct persistence surface, if any, remains defined by the Combat Core GDD; Character Progression does not persist Combat runtime current resources.
...
| **B8. First-run: no save exists (new game / new character)** | Not an error. Load request resolves to the *no-save-to-load* first-run path per Rule 14. Save/Load receives Character Creation's validated `InitialCharacterRecord`, invokes ADR-0004 required first-save materializers including `CharacterProgressionFirstSaveMaterializer` and `InventoryFirstSaveMaterializer`, and writes the complete initial record atomically. **No event emitted.** | First-run is an expected state, not a failure. Emitting any event for this path would conflate normal first-session bootstrap with a data-loss symptom (which is precisely what B9 `SaveMissing` preserves the distinction for). |
...
| `FirstSaveMaterializationFailed` | `Writing` on first-save path — an ADR-0004 required first-save materializer rejects, fails, or exceeds its budget before producing required payloads | Transition to `WriteFailed`; no bytes written and slot remains uninitialized | `SaveFailedEvent(FirstSaveMaterializationFailed)` → caller; Menus/first-run surface handles retry/cancel | Yes — retry while the creation flow remains locked; existing overwritten record remains intact until replacement commit succeeds |
| `DownstreamSaveBarrierUnresolved` | `Writing` — a declared downstream save-stability hook does not settle within its bounded budget or transition-save deadline | Transition to `WriteFailed`; no bytes written | `SaveFailedEvent(DownstreamSaveBarrierUnresolved)` → caller; if Transition Save, WS handles as save failure / ZoneError path | Yes — retry after the gameplay transaction settles |
...
| **Character Creation** (§6) | S/L depended on by | ADR-0004 `InitialCharacterRecord` with `local_character_id` on first save; deserialised Player State on load. | **Hard** | Character Creation owns character-record schema and identity generation; Save/Load owns persistence and active-record identity context. |
| **Character Progression** (§8) | S/L depended on by | ADR-0004 `CharacterProgressionFirstSaveMaterializer`; ADR-0002 `ProgressionSaveBarrier`; whitelisted progression state (`progression_schema_version`, `class_id`, `current_level`, `total_xp`, `spell_eligibility_tier`) persisted and hydrated; ADR-0003 `CombatProgressionBaselineSnapshot` produced before Combat actor hydration. | **Hard** | Character Progression owns schema, validation, materialization, permanent baseline computation, and save-eligible stability; Save/Load owns serialisation, HMAC, versioning, active-record identity context, load ordering, and failure surfacing. |
| **NPC System** (§4) | S/L depended on by | `NpcSourceLifecycleSaveBarrier`; NPC-owned `NpcRecord` data and progression source lifecycle records persisted and hydrated; invalid NPC hydration maps to `LoadRejected(HydrationFailed)`. | **Hard** | NPC System owns NPC schema, source lifecycle schema, and validation; Save/Load owns serialisation, HMAC, versioning, load ordering, and failure-class routing. |
| **Inventory & Item Economy** (§9) | S/L depended on by | ADR-0004 `InventoryFirstSaveMaterializer`; items, currency, faction tokens persisted and hydrated; Inventory-owned hydration validation result. | **Hard** | Inventory owns item schema, first-save materialization, validation, and current state; Save/Load owns serialisation, active-record identity context, first-save orchestration, and failure surfacing. |
...
- **Combat Core** (§7, graph-listed at [systems-index.md:35](systems-index.md)) — **Hard (graph).** Participates indirectly via Character Progression (settled XP updates and ADR-0003 progression-derived actor level / permanent health/mana maxima for load hydration), NPC System (ADR-0001 durable source lifecycle state for defeated sources), and Death & Corpse Recovery (→ WS's `CorpseRecord`). Save writes consult Character Progression's `ProgressionSaveBarrier` and NPC System's `NpcSourceLifecycleSaveBarrier` before serializing XP and source state; Combat actor hydration/build occurs after Character Progression publishes `CombatProgressionBaselineSnapshot`; Combat Core's own direct persistence surface, if any, remains defined in its own GDD.
...
**GIVEN** no save file at the expected location AND the slot is not initialised per the backend's status mechanism (the specific mechanism — marker file, metadata field, save index entry — is implementation-level per Rule 14 + storage-backend ADR), **WHEN** the player starts a new character, **THEN** the load request resolves to the *no-save-to-load* first-run path; no `LoadRejected` event emitted; Save/Load receives Character Creation's validated `InitialCharacterRecord`, invokes required ADR-0004 first-save materializers including `CharacterProgressionFirstSaveMaterializer` and `InventoryFirstSaveMaterializer`, and the first successful save writes both the seed and required materialized payloads.
...
**H-SL-FM-DownstreamSaveBarrierUnresolved**
**GIVEN** a test double for Character Progression or NPC System holds its declared save-stability barrier beyond the bounded budget, **WHEN** Save/Load is in `Writing`, **THEN** state transitions to `WriteFailed`; `SaveFailedEvent(DownstreamSaveBarrierUnresolved)` emits; no bytes are written; the prior committed save remains intact.
*Integration (PlayMode with downstream barrier test double) | gameplay-programmer + engine-programmer + qa-tester | T1-blocking*

**H-SL-FM-FirstSaveMaterializationFailed**
**GIVEN** Character Creation provides a valid `InitialCharacterRecord` but an ADR-0004 required first-save materializer, such as `CharacterProgressionFirstSaveMaterializer` or `InventoryFirstSaveMaterializer`, rejects, fails, or exceeds its budget, **WHEN** Save/Load handles the first-save path, **THEN** state transitions to `WriteFailed`; `SaveFailedEvent(FirstSaveMaterializationFailed)` emits; no bytes are written; the slot remains uninitialized; and retry uses the same pending `InitialCharacterRecord.local_character_id` while the creation flow remains locked.
*Integration (PlayMode with first-save materializer test double) | gameplay-programmer + engine-programmer + qa-tester | T1-blocking*
...
**GIVEN** a valid save fixture containing Character Progression and Combat player resource state, **WHEN** Save/Load runs `Resuming`, **THEN** Character Progression hydrates and validates first, publishes ADR-0003 `CombatProgressionBaselineSnapshot` with `combat_actor_level = current_level`, `permanent_max_health`, and `permanent_max_mana`, and Combat Core hydrates or builds the player combat actor only after receiving that Combat-scoped snapshot.
...
**GIVEN** a Combat `PlayerKillCreditEvent` and a Manual Save or Transition Save trigger are delivered on the same frame, **WHEN** Save/Load enters `Writing`, **THEN** it invokes Character Progression's `ProgressionSaveBarrier` and NPC System's `NpcSourceLifecycleSaveBarrier` before reading progression or NPC source-lifecycle state; the serialized `total_xp`, `current_level`, `spell_eligibility_tier`, and `NpcSourceLifecycleRecord` match post-barrier runtime state; if either barrier exceeds its bounded budget or the transition-save deadline, Save/Load emits `SaveFailedEvent(DownstreamSaveBarrierUnresolved)` and writes no record.
*Integration (PlayMode event-log assertion across Save/Load + Character Progression + NPC System + Combat Core) | gameplay-programmer + engine-programmer + qa-tester | T1-blocking*
...
| H-SL-FM-DownstreamSaveBarrierUnresolved | Failure class | Integration | gameplay-programmer, engine-programmer, qa-tester | Yes |
| H-SL-FM-FirstSaveMaterializationFailed | Failure class | Integration | gameplay-programmer, engine-programmer, qa-tester | Yes |
...
| H-SL-CS-CharacterProgression-SaveBarrier | Character Progression + NPC pending kill/source save barriers | Integration | gameplay-programmer, engine-programmer, qa-tester | Yes |
...
**Total: 40 criteria. 37 T1-blocking, 3 advisory-at-T1 (H-SL-R10a, H-SL-R10b, H-SL-FM-MigrationStepFailed — all promote to T1-blocking at first v1→v2 schema bump).**
