...
9. **Session resume sequencing.** The only hard upstream ordering rule is from [world-structure.md](world-structure.md) Rule 13 + H-CR-13b: **`SessionResumeEvent(real_elapsed_seconds, last_exit_timestamp_utc)` must fire before any `ZoneActiveEvent`** in the session-load sequence. Save/Load's load path must therefore deliver `last_exit_timestamp_utc` to WS early enough that WS can publish `SessionResumeEvent` before publishing the first `ZoneActiveEvent`. **The internal ordering between `SessionResumeEvent` dispatch and the hydration of other gameplay-state categories** (Character Progression, Inventory, Faction State Simulation, Faction Reputation, etc.) **is not locked by this GDD.** Each downstream subscriber's Rule 13 catch-up handler specifies its own state-readiness requirements in its own GDD — e.g., if Faction State Simulation's catch-up handler requires its Faction State to be hydrated before it can run, Faction State Simulation's GDD declares that dependency and Save/Load's load path must satisfy it. Save/Load owns only the pre-`ZoneActiveEvent` guarantee; subscriber-state readiness is a downstream-GDD contract.
...
| `Writing` | Write request dequeued from `Idle` (or from the mutex queue as a follow-on) | `SaveWriteConfirmed` emitted → `Idle`; or fatal write error → `WriteFailed` | Read current in-memory Player State, World State, Session Metadata (including WS-provided `last_exit_timestamp_utc`, `PlayerZoneMembership`, `CorpseRecord` if present); serialise in version-stamp-first order; compute HMAC over full payload; commit atomically per Rule 7. Write-attempt context (trigger type, timestamp) held in transient memory for the outcome event. |
...
| `Resuming` | `Loading` (or `Migrating` → `Loading`) completes successfully with full deserialised state | Session Metadata delivered to WS + downstream systems notified they may proceed → `Idle`; or **synchronous hydration failure in a downstream system during delivery → `LoadRejected(HydrationFailed)` per §Edge Cases D3** | **Rule 9 hard ordering (narrowed)**: delivers Session Metadata (`last_exit_timestamp_utc`) to World Structure **before any `ZoneActiveEvent` fires** — this is the only hard ordering locked by Save/Load. Delivers deserialised Player State and World State to their owning systems (Character Progression, Inventory, Faction State Simulation, Faction Reputation, Death & Corpse Recovery). The ordering between Session Metadata delivery and gameplay-state category hydration is not locked by this state; individual downstream systems specify their own state-readiness requirements in their GDDs (per Rule 9). |
...
| **Character Progression** (§8) | Deserialised Character Progression state on load (XP, level, spells, learned abilities). | Current Character Progression state on each save trigger. | Character Progression owns the progression schema; Save/Load owns serialisation. | **Hard** |
...
- **Combat Core** (§7) participates in saves *indirectly* — XP updates flow through Character Progression; deaths flow through Death & Corpse Recovery → WS's `CorpseRecord`. Combat Core's own direct persistence surface, if any (including whether health / mana / other combat-facing values persist across sessions), is defined in the Combat Core GDD when authored. This GDD does not preempt that boundary; the save-integrity rule's §What Saves exclusion list (engine internals, derived/cached values, runtime handles) does not categorise specific gameplay values as non-persistent.
...
| **Character Progression** (§8) | S/L depended on by | XP, level, spells, learned abilities — persisted and hydrated. | **Hard** | Character Progression owns progression schema; Save/Load owns serialisation. |
...
- **Combat Core** (§7, graph-listed at [systems-index.md:35](systems-index.md)) — **Hard (graph).** Participates indirectly via Character Progression (XP updates) and Death & Corpse Recovery (→ WS's `CorpseRecord`). Combat Core's own direct persistence surface, if any, is defined in its own GDD when authored; this GDD does not preempt that boundary.
...
**Total: 36 criteria. 33 T1-blocking, 3 advisory-at-T1 (H-SL-R10a, H-SL-R10b, H-SL-FM-MigrationStepFailed — all promote to T1-blocking at first v1→v2 schema bump).**
