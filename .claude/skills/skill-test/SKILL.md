...
argument-hint: "static [skill-name | all] | spec [skill-name] | category [skill-name | all] | audit [--dry-run]"
user-invocable: true
...
Validate skill files for static structure, tool-contract correctness, behavioral spec coverage, category consistency, and audit readiness.
...
- static [skill|all]: frontmatter and structural lint
- spec [skill]: behavioral checklist review
- category [skill|all]: taxonomy/category validation
...
- skills/reviews/skill-test-[mode]-[YYYYMMDD].md
...
1. Discover target skills and parse frontmatter.
2. Check required fields, invocation hints, user-invocable flag, allowed tools, actual tool mentions, write policy, path safety, missing-file behavior, and final output rules.
3. Classify failures as Critical, High, Medium, Low.
4. Write a report only; do not modify skill files.
5. In spec mode, evaluate whether the skill has deterministic phase order and protected-write handling.
