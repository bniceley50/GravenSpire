...
argument-hint: "[system-name | all | scaffold] [--dry-run]"
user-invocable: true
...
Generate engine- and project-specific test helper libraries that reduce boilerplate while preserving clear assertions and production contracts.
...
- all: general helper library
- scaffold: base directory and README only
...
1. Inspect existing test style, framework, fixtures, naming, and engine-specific runner requirements.
2. Generate helpers only where repeated patterns already exist or GDD/ADR contracts justify them.
3. Avoid hiding important assertions behind overly broad helpers.
4. Do not overwrite existing helper files; create proposed additions or append clearly separated functions with confirmation.
5. Include usage examples and limitations.
