...
argument-hint: "[path-to-design-doc] [--depth full|lean|solo] [--dry-run]"
user-invocable: true
...
Review a GDD or design document for completeness, consistency, implementation readiness, scope control, traceability, and handoff quality.
...
- full: complete design review
- lean: structural and blocking issues only
- solo: no subagent review
...
- design/reviews/[doc-slug]-design-review-[YYYYMMDD].md
...
1. Normalize the target path and reject unsafe paths.
2. Check required sections, acceptance criteria, dependencies, content counts, formulas, UX/audio/art hooks, and implementation handoff details.
3. Compare names and mechanics against registries and neighboring GDDs.
4. Classify findings as Blocking, High, Medium, Low.
5. Use Task reviewers only in full mode and only for materially relevant specialist viewpoints.
...
- No design document path was supplied and no obvious current GDD can be inferred.
