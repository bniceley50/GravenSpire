...
argument-hint: "[screen/flow name] or 'hud' or 'patterns' [--dry-run]"
user-invocable: true
...
Author a UX spec for a screen, flow, HUD, or interaction pattern using player goals, GDD requirements, accessibility, information hierarchy, and implementation handoff needs.
...
- screen/flow name: create one UX spec
- hud: HUD spec
- patterns: interaction pattern library
...
- design/ux/[slug]-ux-spec.md
- design/ux/hud-design.md
...
1. Resolve target surface and user goals.
2. Extract relevant systems, player states, content, input constraints, accessibility needs, and platform constraints.
3. Define user flow, layout hierarchy, states, error/empty/loading states, input behavior, feedback, copy needs, analytics, and QA checks.
4. Use UX Task review when available for full mode; revise once for blocking concerns.
5. Write spec and list art/implementation dependencies.
...
- No target screen/flow/HUD/pattern can be inferred.
