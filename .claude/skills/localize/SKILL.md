...
argument-hint: "[scan|extract|validate|status|brief|cultural-review|vo-pipeline|rtl-check|freeze|qa] [--dry-run]"
user-invocable: true
...
Find hardcoded strings, manage localization tables, validate translated content structure, and produce localization QA reports without changing player-facing text semantics without review.
...
- scan: identify localizable strings
- extract: create/update source string table
- validate: check tables and missing keys
- language:<code>: focus one locale
...
- localization/source_strings.csv
- localization/localization-report-[YYYYMMDD].md
- localization/missing-keys-[YYYYMMDD].md
...
1. Use safe grep/Bash discovery to find literal strings and existing localization keys.
2. Classify strings as player-facing, debug/dev-only, dynamic format, accessibility label, or not localizable.
3. Extract keys with stable namespaces and source references; do not delete existing keys automatically.
4. Validate table completeness, duplicate keys, missing placeholders, and inconsistent variable usage.
5. Write reports and source tables only in approved localization paths.
