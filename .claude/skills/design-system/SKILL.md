...
argument-hint: "<system-name> [--review full|lean|solo] [--dry-run]"
user-invocable: true
...
Author or revise a single system GDD with enough specificity for architecture, epic creation, story decomposition, QA planning, and implementation.
...
- <system-name>: create or update one system GDD
- --review full: run specialist review
- --review lean: self-review only
- --review solo: no subagents
...
1. Resolve the system name and locate existing GDD context before asking questions.
2. Use TodoWrite to track required sections: summary, player fantasy, loop, rules, data, content, UI/audio/art hooks, dependencies, acceptance criteria, tuning, risks, and QA notes.
3. Draft from existing context first; ask only for unresolved design choices that materially affect the spec.
4. Write incrementally but never overwrite user-authored sections without confirmation.
5. Run specialist review in full mode and revise once for blocking findings.
...
- No system name can be inferred from invocation or project context.
