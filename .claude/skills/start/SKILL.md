...
argument-hint: "[no arguments] [--dry-run]"
user-invocable: true
...
Orient a new or returning user by detecting current project state and routing them to the next appropriate workflow.
...
- blank: interactive/project-state guided start
- --dry-run: recommend without writing session state
...
- production/session/start-context-[YYYYMMDD].md
...
1. Read project evidence before asking where to begin.
2. Classify project stage and likely next workflow.
3. Ask only when multiple valid workflows are equally plausible or the user goal is ambiguous.
4. Write a small session context note only if useful and not in dry-run.
5. Do not create design, architecture, sprint, or QA artifacts directly; route to the correct skill.
