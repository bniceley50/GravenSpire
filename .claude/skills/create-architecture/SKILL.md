...
argument-hint: "[focus-area: full | layers | data-flow | api-boundaries | adr-audit] [--review full|lean|solo] [--dry-run]"
user-invocable: true
...
Author the master architecture blueprint from GDDs, ADRs, engine reference docs, registries, and technical preferences before implementation proceeds.
...
- full: complete architecture blueprint
- layers: layer and module map only
- data-flow: ownership and signal/data flow only
- api-boundaries: public interfaces and forbidden access only
- adr-audit: identify missing ADRs needed before architecture can be finalized
...
- docs/architecture/architecture-review-notes-[YYYYMMDD].md
...
1. Load all Approved or implementation-relevant GDDs and summarize required systems.
2. Map systems into layers, ownership boundaries, data flows, APIs, event/signal contracts, persistence, platform constraints, and test strategy.
3. Cross-check every architectural claim against accepted ADRs and registry entries; mark proposed or missing ADRs as blockers rather than silently accepting them.
4. Use Bash only for safe repository discovery when Grep/Glob are insufficient.
5. Run TD review in full mode and revise once for blocking concerns.
...
- No GDDs or concept documents exist and no architecture scope was supplied.
