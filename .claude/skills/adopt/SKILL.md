...
argument-hint: "[focus: full | gdds | adrs | stories | infra] [--dry-run]"
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, Edit, AskUserQuestion
agent: technical-director
---

# Adopt — Brownfield Template Migration

Audit an in-progress project for compatibility with the studio template and produce a migration plan that existing artifacts can be brought into the expected GDD, ADR, story, QA, and production formats without losing project history.
...
- full: audit all artifact families
- gdds: audit GDD structure and traceability only
- adrs: audit ADR structure, status, dependency, and registry coverage only
- stories: audit epics, stories, sprint state, and evidence paths only
- infra: audit registries, manifests, stage files, and skill configuration only
...
- docs/adoption/adoption-audit-[YYYYMMDD].md
- docs/adoption/migration-plan-[YYYYMMDD].md
...
1. Classify the requested focus and discover artifacts without assuming greenfield structure.
2. Validate each artifact against the current template contract: required headings, status fields, traceability IDs, dependency links, and evidence paths.
3. Classify every gap as Blocking, High, Medium, or Low based on whether downstream skills would fail or merely lose detail.
4. Group fixes into safe migration phases: source-of-truth fixes, traceability fixes, planning fixes, evidence fixes, and cleanup.
5. Prefer non-destructive migrations that add missing fields or create migration notes over rewriting legacy content.
...
- Executive adoption verdict
- Artifact inventory
- Blocking compatibility gaps
- Migration plan by phase
- Files safe to touch automatically
...
- Requested focus has no matching artifacts and no clear fallback.
- The repository appears outside the expected project root.
